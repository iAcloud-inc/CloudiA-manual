...
## 멤버 목록
**멤버 목록** 화면에서는 현재 시스템에 등록된 모든 사용자(멤버)를 확인하고 관리할 수 있습니다. 화면 상단의 도구 모음에서 새로운 멤버를 [생성]할 수 있으며, 목록에서 멤버를 하나 선택하면 [편집], [삭제] 및 기타 계정 관련 기능을 사용할 수 있습니다. 예를 들어 한 명의 멤버가 선택되면 기본 도구 모음의 [편집] 버튼이 활성화되어 해당 멤버의 정보를 수정할 수 있고, [삭제] 버튼을 통해 해당 멤버 계정을 시스템에서 **영구 삭제**할 수 있습니다. 또한 선택된 멤버에 대해 **비밀번호 변경**, **임시 잠금 해제**, **OTP 재등록** 등의 추가 작업을 수행할 수 있습니다. 이러한 추가 기능은 도구 모음 우측의 **더보기** 드롭다운 메뉴를 통해 제공됩니다. 멤버를 선택하지 않은 경우에는 [전체 임시 잠금 해제] 버튼을 사용하여 **현재 잠겨 있는 모든 멤버 계정**을 일괄 정상화할 수도 있습니다. 각 멤버의 **이름**을 클릭하면 해당 멤버의 **상세 정보** 화면으로 이동합니다.
...
### 멤버 비밀번호 변경
멤버 계정의 비밀번호를 관리자 권한으로 재설정할 수 있습니다. 멤버 목록에서 대상 사용자를 선택한 후 **더보기** 메뉴의 **비밀번호 변경** 기능을 이용하면, 새로운 비밀번호를 입력하여 해당 사용자의 로그인 비밀번호를 변경할 수 있습니다. 비밀번호는 현재 설정된 **비밀번호 정책**을 준수해야 합니다. 비밀번호 변경이 완료되면 해당 사용자에는 로그인 후 새로운 비밀번호로 변경해야 합니다.

### 멤버 임시 잠금 해제
시스템의 계정 잠금 정책에 의해 **임시 잠금** 상태가 된 사용자는 일정 시간 동안 로그인할 수 없으나, 관리자는 이를 수동으로 해제할 수 있습니다. 멤버 목록에서 **임시 잠금**으로 표시된 사용자를 선택한 뒤 **더보기** 메뉴의 **임시 잠금 해제** 기능을 사용하면, 확인 대화상자가 나타나 해당 계정의 잠금을 즉시 해제할 수 있습니다. 확인 창에는 선택된 사용자 정보(이름, ID, 이메일 등)와 “임시 잠금을 해제하시겠습니까?”라는 메시지가 표시됩니다. 확인을 진행하면 해당 멤버 계정 상태가 **정상**으로 복구되어 즉시 로그인할 수 있습니다.

> 💡 **전체 임시 잠금 해제:** 멤버를 선택하지 않은 상태에서 목록 상단의 **[전체 임시 잠금 해제]** 버튼을 클릭하면, 현재 **임시 잠금** 상태인 모든 멤버 계정을 한 번에 정상화할 수 있습니다. 이 기능을 사용하면 다수의 잠긴 계정을 일괄 해제할 수 있습니다.

### 멤버 OTP 재등록
만약 사용자가 OTP(2차 인증)를 분실했거나 재등록이 필요한 경우, 관리자는 **OTP 재등록** 기능을 통해 해당 사용자의 OTP를 초기화할 수 있습니다. 멤버 목록에서 대상을 선택한 후 **더보기** 메뉴에서 **OTP 재등록**을 클릭하면 해당 사용자의 기존 OTP 설정이 삭제되고, **다음 로그인 시 otp 활성화 여부에 따라 새로운 OTP 등록 과정**을 거치게 됩니다. 

### 멤버 삭제
멤버 계정을 삭제하려면, 목록에서 대상 계정을 선택하고 **삭제** 버튼을 누르면 해당 사용자 계정이 시스템에서 완전히 제거되며, 더 이상 로그인하거나 프로젝트에 접근할 수 없게 됩니다.
