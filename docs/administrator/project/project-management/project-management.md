...
1. 프로젝트 상세 화면의 **멤버** 탭에서 **`편집`** 버튼을 클릭하여 멤버 편집 화면 진입합니다. 
2. **`역할 추가`** 버튼을 클릭하여 맴버별 역할을 추가하거나 **`삭제`** 버튼을 클릭하여 맴버별 역할을 삭제합니다. 

> ⚠️ **주의:** 시스템 기본 역할 중 하나인 `admin` 역할의 경우 역할 추가가 불가능합니다.
 
> 💡 **참고:** 역할 추가 버튼은 최대 멤버 수까지만 활성화됩니다. 
