...
관리자 방화벽은 네트워크 노드에 적용되는 인바운드 규칙을 관리하는 메뉴입니다. 규칙은 목록의 위에서 아래 순서로 평가되며, 마지막 기본 규칙은 `*`로 표시됩니다.
...
- **필드 편집:** 각 규칙 행에서는 아래와 같은 필드를 편집할 수 있습니다 (비활성화 상태의 규칙만 편집 가능).
  - **프로토콜:** 드롭다운에서 트래픽 프로토콜을 선택합니다. **TCP/UDP**를 선택하면 포트 필드에 기본값으로 `0-65535`(모든 포트 범위)가 설정되고, **모든 트래픽/모든 ICMP**를 선택하면 포트 필드가 자동으로 `전체`로 지정됩니다.
  ...
  - **목적지 포트:** 허용 또는 차단할 대상 포트 번호를 입력합니다. 단일 포트 번호(예: `22`)나 범위(`1024-2048`)로 입력할 수 있으며, 모든 포트를 대상으로 할 경우 **0-65535**로 설정합니다. 포트 번호는 **1**에서 **65535** 사이의 값이어야 하고, 범위의 경우 시작 포트가 끝 포트보다 작거나 같아야 합니다.
  ...
  - **활성화 여부:** 토글 형태의 스위치로 규칙의 적용 여부를 설정합니다. 스위치를 **켜짐(활성화)**으로 하면 해당 규칙이 방화벽에 적용되고, **꺼짐(비활성화)**으로 하면 적용되지 않습니다. 신규 추가된 규칙은 기본적으로 비활성화 상태이며, 편집 완료 후 필요 시 스위치를 켜서 활성화합니다.
...
| **목적지 포트**    | `1`–`65535` 단일 포트 또는 범위 <br/>예: `22`, `1024-2048`, `0-65535`(전체 포트) | (필수) `TCP/UDP`일 때만 직접 입력 가능 <br/> 포트 번호는 `1`–`65535` 범위 | `모든 트래픽`/`모든 ICMP` 선택 시 자동 `전체` |
| **허용/거부**    | `허용` / `거부`                                                         | (필수) <br/> `허용`: 조건에 일치하는 트래픽 통과(ACCEPT) <br/> `거부`: 트래픽 차단(DROP)        | 룰 동작 설정                         |
| **설명**         | 최대 200자                                                             | (선택) 규칙 설명 및 메모                                 | -                               |
| **활성화 여부** | `켜짐(활성화)` / `꺼짐(비활성화)`                                              | 활성화 시 저장 후 규칙이 적용됨. <br/> 일반적으로 **비활성화 상태의 규칙만 편집 가능**   | 규칙 적용/미적용 상태 제어                 |

### 동작 규칙
- 새 규칙은 기본값으로 추가되며, 필요한 값으로 수정한 뒤 저장합니다.
- `TCP` 또는 `UDP`를 선택하면 포트 값은 기본적으로 전체 범위로 채워집니다.
- `모든 ICMP` 또는 `모든 트래픽`을 선택하면 포트는 `전체`로 고정됩니다.
- 규칙은 드래그 앤 드롭으로 순서를 바꿀 수 있습니다.
- 활성화된 규칙은 수정하거나 삭제할 수 없습니다. 먼저 비활성화해야 합니다.
- `*` 기본 규칙은 시스템 규칙이므로 수정하거나 삭제할 수 없습니다.

> 💡 **참고:** 
> - Cloud:iA 웹 콘솔 접근은 방화벽 설정과 무관하게 항상 허용됩니다.
> - 규칙들은 드래그 앤 드롭으로 **순서를 변경**할 수 있습니다. 규칙 목록 상단(번호 1)부터 차례로 적용되며, 어떤 규칙에 트래픽이 일치하여 처리되면 이후 규칙들은 평가되지 않습니다. 특정 트래픽을 허용하려면 보다 상위 우선순위(작은 번호)에 허용 규칙을 추가하고, 범용 차단 규칙은 하위 우선순위로 배치해야 합니다.

> ⚠️ **주의:**
> * **활성화된 규칙은 편집하거나 삭제할 수 없습니다.** 규칙을 수정하거나 삭제하려면 먼저 해당 규칙을 **비활성화**해야 합니다. 또한, 기본 규칙(‘*’)은 시스템 기본값이므로 수정/삭제가 불가합니다.
