...
> ⚠️ **주의: 초기 설치 상태의 허용 규칙**
> 초기 설치 직후에는 **모든 ICMP**와 **SSH(TCP 22)** 트래픽을 소스 CIDR `0.0.0.0/0`(모든 IP)에서 허용하는 규칙이 적용된 상태입니다. SSH 접근을 제한하거나 보안을 강화하려면 해당 규칙의 **소스 CIDR을 사내망·VPN 대역 등 신뢰할 수 있는 범위로 좁혀서** 사용하시기 바랍니다.
> * 입력 형식은 `10.0.0.0/8`, `203.0.113.0/24`와 같은 CIDR 표기를 따릅니다. 이 값들은 표기 예시이므로 **그대로 입력하지 말고 실제로 사용 중인 사내망·VPN 대역으로 바꿔 입력**해야 합니다.
> * 소스 CIDR은 `편집` 화면에서 규칙을 **비활성화**한 뒤 수정하고 다시 활성화하는 순서로 변경합니다.
> * 변경 전에 관리 작업에 사용하는 IP가 새 대역에 포함되는지 확인해야 합니다. 포함되지 않으면 저장 직후 해당 노드로 SSH 접속을 할 수 없게 됩니다.

### 웹콘솔 접근 허용 대역
방화벽 목록 화면 상단에는 **웹콘솔 접근 허용 대역**이 표시됩니다. Cloud:iA 웹콘솔에 접근을 허용하는 출발지 IP 대역을 조회용으로 보여줍니다. 값은 텍스트로만 표시되며, 변경하려면 `편집` 화면으로 이동해야 합니다.

| 항목 | 설명 |
|---|---|
| **접근 허용 CIDR** | 웹콘솔 접근이 허용된 출발지 IP 대역입니다. |

> 💡 **참고:** 제목 옆의 안내 아이콘에 마우스를 올리면 이 설정의 의미가 표시됩니다. 웹콘솔 접근 허용 대역은 방화벽 규칙과 별도로 관리되며, 항상 방화벽보다 우선 적용됩니다.
...
- `저장`: 변경사항을 적용합니다. 저장 시 방화벽 규칙과 **웹콘솔 접근 허용 대역(접근 허용 CIDR)**이 함께 저장됩니다. 접근 허용 CIDR이 유효하지 않거나 변경사항이 없으면 `저장` 버튼이 비활성화됩니다.
...
### 웹콘솔 접근 허용 대역
방화벽 규칙 편집 화면에서는 **웹콘솔 접근 허용 대역**의 **접근 허용 CIDR**을 직접 입력하거나 수정할 수 있습니다.

- **접근 허용 CIDR(필수):** Cloud:iA 웹콘솔 접근을 허용할 출발지 IP 대역을 CIDR 형식으로 입력합니다. (예: `10.0.0.0/16`, `0.0.0.0/0`)
  - 유효한 CIDR 형식이 아니면 **유효한 CIDR 값을 입력하세요.** 오류 메시지가 표시되며, 올바른 값을 입력해야 `저장`할 수 있습니다.

여기에 지정한 **접근 허용 CIDR에 포함된 IP 대역에서만** Cloud:iA 웹콘솔에 접근할 수 있습니다. 범위 밖의 IP에서는 웹콘솔 접속이 차단됩니다. 모든 IP의 접근을 허용하려면 `0.0.0.0/0`을 입력합니다.

> 💡 **이 설정의 의미:** 웹콘솔 접근을 신뢰할 수 있는 IP 대역(예: 사내망, VPN 대역)으로 제한하여 외부의 무단 접근을 차단할 수 있습니다. 이 설정은 방화벽 규칙과 별도로 관리되며 **항상 방화벽 규칙보다 우선 적용**되므로, 방화벽 규칙이 어떻게 설정되어도 허용된 대역에서는 웹콘솔 접속이 보장되고 허용되지 않은 대역은 차단됩니다.

> ⚠️ **주의:** 접근 허용 CIDR에는 **현재 웹콘솔에 접속 중인 관리자의 IP 대역을 반드시 포함**해야 합니다. 본인의 IP가 포함되지 않은 대역으로 저장하면 저장 직후 웹콘솔에 접근할 수 없게 되므로 주의해야 합니다.
