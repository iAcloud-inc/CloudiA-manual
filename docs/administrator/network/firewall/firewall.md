...
> 💡 **참고:** Cloud:iA 웹콘솔 접근은 방화벽 규칙과 별도로 **웹콘솔 접근 설정**의 소스 CIDR로 제어됩니다. 웹콘솔 접근 설정은 방화벽 규칙과 별도로 관리되며 **항상 방화벽보다 우선 적용**되므로, 방화벽 규칙으로 모든 포트를 차단하더라도 허용된 소스 CIDR 범위에서는 웹콘솔에 접속할 수 있습니다.

### 웹콘솔 접근 설정
방화벽 규칙 편집 화면에서는 방화벽 규칙과 별도로 **웹콘솔 접근 설정**의 **소스 CIDR**을 직접 입력하거나 수정할 수 있습니다. 여기에 지정한 **소스 CIDR에 포함된 IP 대역에서만** Cloud:iA 웹콘솔에 접근할 수 있으며, 범위 밖의 IP에서는 접속이 차단됩니다. 모든 IP의 접근을 허용하려면 `0.0.0.0/0`을 입력합니다.

| 필드            | 허용 값                                                  | 검증 규칙                                                                | 비고                                          |
| ----------------|----------------------------------------------------------| -------------------------------------------------------------------------|---------------------------------------------|
| **소스 CIDR**  | IPv4 CIDR <br/>예: `0.0.0.0/0`, `203.0.113.0/24`, `10.0.0.0/16` | (필수) 유효한 IPv4 CIDR 형식이어야 함 <br/> 형식이 올바르지 않으면 **유효한 CIDR 값을 입력하세요.** 오류가 표시되며 저장할 수 없음 <br/> `0.0.0.0/0`은 모든 IP 대역을 의미 | 웹콘솔 접근을 허용할 출발지 IP 대역             |
