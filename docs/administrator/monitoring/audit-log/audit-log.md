...
|---|---|
| ID | 로그 식별자 (고유 번호) |
| 메서드 | 요청한 API의 HTTP 메서드 (POST/PUT/DELETE) |
| 경로 | 요청한 API 엔드포인트 경로 |
...
> 💡 **참고:** 감사 로그 화면의 데이터는 조회 전용이며 수정이나 삭제가 불가능합니다.

> 💡 **참고:** 조회 결과가 없는 경우 *“리스트 데이터가 존재하지 않습니다.”*라는 안내가 표시됩니다.

> 💡 **참고:** 경로를 클릭하면 해당 요청의 상세 내용을 볼 수 있습니다. 상세 화면에서 요청/응답 바디까지 확인할 수 있습니다.
