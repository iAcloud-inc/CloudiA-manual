...
|---|---|
| ID | 로그 식별자 (고유 번호) |
| 메서드 | 요청한 API의 HTTP 메서드 (POST/PUT/DELETE). 인스턴스 콘솔 접속·종료 기록은 `CONSOLE`로 표시됩니다 |
| 경로 | 요청한 API 엔드포인트 경로 |
...
인스턴스 콘솔(웹 콘솔) 접속은 접속 시점과 종료 시점이 각각 한 건씩 기록됩니다.  
메서드 필터를 `CONSOLE`로 두면 콘솔 접속 기록만 모아 볼 수 있습니다.
