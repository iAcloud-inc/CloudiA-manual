...
|---|---|
| ID | 로그 식별자 (고유 번호) |
| 메서드 | 요청한 API의 HTTP 메서드 (POST/PUT/DELETE). 그래픽 콘솔 접속·종료 기록과 콘솔 로그 다운로드 기록은 `CONSOLE`으로, 사용자 요청 없이 백엔드가 남기는 기록(노드 연결·페일오버·유지관리)은 `SYSTEM`으로 표시됩니다 |
| 경로 | 요청한 API 엔드포인트 경로 |
| 사용자 이름 | 액션을 수행한 사용자 이름 |
...
| 필터 유형 | 설명 |
|---|---|
| 날짜 범위 | 조회할 기간의 시작일과 종료일을 지정 (기본값 최근 7일 내 로그) |
| 결과 필터 | 성공, 실패 또는 전체 결과 중 선택하여 요청 결과별로 필터링 |
| 메서드 필터 | POST, PUT, DELETE, CONSOLE, SYSTEM 또는 전체 중 선택하여 요청 메서드별로 필터링 (`SYSTEM`은 사용자 요청 없이 백엔드가 남기는 기록, 예: 노드 연결·페일오버·유지관리) |
...
메서드 필터를 `CONSOLE`로 두면 인스턴스 콘솔 관련 기록을 모아 볼 수 있는데, 이 안에는 서로 다른 세 종류의 기록이 함께 섞여 나옵니다.

- 그래픽 콘솔(VNC) **접속** 기록
- 그래픽 콘솔 **종료** 기록
- **콘솔 로그 다운로드** 기록

세 종류는 경로(URI)로 구분합니다. 콘솔 로그 다운로드 기록은 경로가 `.../console-log/download`로 끝나고, 접속·종료 기록은 각각 `.../connected`, `.../disconnected`로 끝납니다.

> ⚠️ **주의:** `CONSOLE`로 남는 접속·종료 기록은 **그래픽 콘솔에 한정**됩니다. 직렬 콘솔은 접속 요청이 일반 `POST` 기록으로 남고 **종료 기록은 남지 않습니다.** 따라서 직렬 콘솔 사용 여부를 메서드 `CONSOLE` 필터로 판단하면 안 되며, 직렬 콘솔에서 무엇이 오갔는지는 감사 로그가 아니라 콘솔 로그(출력 기록)로 확인합니다.

### 접속·종료 기록

그래픽 콘솔 접속은 접속 시점과 종료 시점이 각각 한 건씩 기록됩니다.

종료 기록의 상세 화면에는 접속 시작 일시, 종료 일시, 연결 지속 시간(초), 종료 사유가 함께 표시되므로  
한 건만 열어도 접속 구간 전체를 확인할 수 있습니다.

브라우저를 강제로 종료한 경우에도 종료 기록이 남습니다. 콘솔 창을 열었지만 실제로 접속하지 않은 경우에는 기록되지 않습니다.

> 💡 **참고:** 같은 인스턴스에 여러 사용자가 동시에 접속한 경우에도 각 접속이 별도의 기록으로 구분됩니다.

### 콘솔 로그 다운로드 기록

콘솔 로그에는 직렬 콘솔 세션에서 게스트가 출력한 내용이 담기기 때문에, 조회 권한을 가진 사용자가 다른 사용자가 콘솔에서 수행한 작업 내용을 사후에 열람할 수 있습니다. 이 노출 자체를 막을 수는 없으므로, 대신 누가 콘솔 로그를 내려받았는지를 감사 로그에 기록으로 남겨 추적할 수 있게 합니다.

콘솔 로그 다운로드 기록의 상세 화면에는 다음 정보가 함께 표시됩니다.

- 누가, 언제, 어느 클라이언트 IP에서 내려받았는지
- 내려받은 콘솔 로그의 전체 크기, 상한을 넘어 잘렸는지 여부, 실제로 저장된 바이트 수
...
`CSV 다운로드` 버튼을 클릭하면 설정된 조건(기간, 결과, 메서드, 검색어)을 그대로 적용한 CSV를 생성합니다.  
...
> 💡 **참고:** 이 화면에서도 데이터는 조회 전용이며, 별도의 편집 기능은 제공되지 않습니다.
