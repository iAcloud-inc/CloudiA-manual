...
| `NIC` | 가능 | 추가/편집/삭제 가능. 기본 NIC는 `보안그룹`만 변경 가능하며 삭제 불가 |
...
### Q. ISO를 업로드할 때 `OS 타입`이 자동으로 채워지지 않습니다.
#### 핵심 답변
정상 동작입니다. `OS 타입`은 ISO를 한 가지로 특정할 수 있을 때만 채워집니다. 특정할 수 없는 ISO는 비워 둔 채 사용자가 직접 고르도록 합니다.

#### 점검 포인트
1. Windows Server 설치 미디어 여부. `Windows Server 2016`부터 `2025`까지는 ISO에 담긴 식별 정보가 같아 한 가지로 특정할 수 없습니다.
2. 부팅할 수 없는 데이터 ISO 또는 `virtio-win` 같은 드라이버 CD 여부
3. 목록에 없는 배포판(`Fedora`, `openSUSE` 등) 여부

#### 조치 가이드
1. `OS 타입` 목록에서 실제 ISO에 맞는 값을 직접 선택
2. 다른 파일을 골랐다면 `ISO 파일`을 다시 선택해 자동 채움 재시도
3. 값을 선택한 뒤 `업로드` 버튼 활성화 확인
...
- 같은 서브넷의 가상머신이나 다른 VPC의 퍼블릭 서브넷 가상머신이 플로팅 IP나 포트 포워딩 외부 IP를 거쳐 접속해, 보안그룹이 판정하는 출발지가 사설 IP가 아닌 경우
...
6. 접속하는 쪽이 **같은 서브넷의 가상머신(자기 자신 포함)**이거나 **다른 VPC의 퍼블릭 서브넷 가상머신**이라면, 보안그룹이 판정하는 출발지가 사설 IP가 아닐 수 있습니다. 자세한 내용은 [외부 IP로 접속할 때 보안그룹이 판정하는 출발지 주소](../../project/network/floating-ip/floating-ip.md#외부-ip로-접속할-때-보안그룹이-판정하는-출발지-주소)를 확인합니다.
...
- 비율을 올려도 **인스턴스 하나가 커질 수 있는 한도는 바뀌지 않습니다**. 오버커밋은 여러 인스턴스를 한 노드에 겹쳐 얹는 총량 설정이고, 인스턴스 하나의 `vCPU`·메모리는 노드의 물리 자원이 상한입니다.
