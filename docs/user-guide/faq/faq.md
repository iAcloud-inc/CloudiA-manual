...
- 노드 연결 상태(VDSM/Agent/네트워크)가 비정상인 경우

#### 점검 포인트
1. `프로젝트 > 가상머신 > 인스턴스`에서 대상 VM 상태가 `SCHEDULE_FAILED`인지 확인합니다.
2. `관리자 > 대시보드 > 컴퓨트 대시보드`에서 비정상 노드 수와 에러 상태 인스턴스를 확인합니다.
3. `관리자 > 물리머신 > 컴퓨트 노드`에서 노드별 `할당된 논리 CPU/메모리`, 오버커밋 비율, 연결 상태를 확인합니다.
4. `인스턴스 상세 > 연결된 리소스`에서 파일 시스템/선호도 그룹/AI 가속기 연결 여부를 확인합니다.
...
- 노드 상태 문제: 연결 비정상, 네트워크 이상

#### 점검 포인트
1. VM 실행 조건을 정리합니다: `vCPU`, `메모리`, `보안 유형`, `피닝`, `가속기`, `선호도 그룹`.
2. 후보 노드별 조건 만족 여부를 체크합니다.
3. 후보가 0대인지 확인합니다.
...
### Q. CPU나 메모리가 남아 있는데도 왜 `SCHEDULE_FAILED`가 발생하나요?
#### 원인 요약
- 대시보드 총량 여유와 스케줄러 배치 판단 기준은 다를 수 있습니다.
- 스케줄러는 "전체 여유"가 아니라 "단일 노드에서 동시 충족 가능" 여부를 봅니다.

#### 점검 포인트
1. 남은 자원이 여러 노드에 분산되어 있는지 확인합니다.
2. 오버커밋/기할당/예약 자원으로 실제 배치 가능량이 줄어들었는지 확인합니다.
3. 피닝/선호도/스토리지/가속기 제약으로 후보가 줄었는지 확인합니다.

#### 조치 가이드
1. 가능한 경우 콜드 마이그레이션으로 노드 자원을 재정렬합니다.
2. 오버커밋 정책을 운영 기준 내에서 재검토합니다.
3. 과도한 제약 조건을 완화합니다.

### Q. 스토리지 도메인 문제로도 `SCHEDULE_FAILED`가 발생할 수 있나요?
#### 원인 요약
가능합니다. VM이 참조하는 이미지/디스크/파일 시스템을 노드가 접근하지 못하면 해당 노드는 스케줄 후보에서 제외됩니다.

#### 점검 포인트
1. `관리자 > 스토리지 > 스토리지 도메인`에서 상태를 확인합니다.
2. 대상 VM이 사용하는 이미지/디스크/파일 시스템의 저장 도메인을 확인합니다.
3. 노드별 접근성이 동일한지 확인합니다.

#### 조치 가이드
1. 도메인 연결/마운트/권한 문제를 우선 복구합니다.
2. 필요 시 이미지를 정상 도메인으로 복제해 우회 실행합니다.
3. 복구 후 VM 실행을 다시 시도합니다.
...
### Q. Windows VM은 왜 일반 Linux VM보다 스케줄 실패가 더 잘 날 수 있나요?
#### 원인 요약
Windows 워크로드는 일반적으로 요구 자원(메모리/디스크)과 설치/운영 제약이 커서 후보 노드가 줄어들기 쉽습니다.

#### 점검 포인트
1. Windows VM 스펙이 Linux 대비 큰지 확인합니다.
2. ISO 설치형인지, 드라이버/게스트 에이전트 준비 상태를 확인합니다.
3. 라이브 마이그레이션 제약(Windows, 보안, 피닝, 패스스루)을 함께 확인합니다.

#### 조치 가이드
1. 초기 설치는 권장 스펙으로 시작하고 점진적으로 확장합니다.
2. 설치 후 `virtio-win` 드라이버와 `QEMU Guest Agent`를 정리합니다.
3. 마이그레이션이 잦은 VM은 제약 조건을 최소화해 설계합니다.
...
### Q. Ubuntu VM은 왜 스냅샷 전에 종료 후 생성이 필요한가요?
#### 핵심 답변
...
#### 핵심 답변
`BitLocker` 활성 디스크는 암호화 메타데이터와 블록 상태가 계속 변경되어 스냅샷/이미지 캡처가 실패하거나 안정성이 떨어질 수 있습니다.
...
### Q. Windows ISO와 `virtio-win` ISO는 왜 같은 스토리지 도메인에 있어야 하나요?
#### 핵심 답변
두 ISO를 동일 접근 조건으로 관리하면 설치/재부팅/재스케줄 시 미디어 접근 실패를 줄여 운영 안정성이 높아집니다.

#### 점검 포인트
1. 두 ISO 저장 도메인 확인
2. VM 실행 노드에서 두 ISO 접근 조건 동일 여부 확인
3. 장애 복구 시 동일 접근성 유지 여부 확인

#### 조치 가이드
1. 설치 ISO와 드라이버 ISO를 동일 도메인으로 표준화
2. 도메인 변경 시 테스트 VM으로 사전 검증

### Q. ISO VM은 왜 CEPH 스토리지 도메인에서 생성할 수 없나요?
#### 핵심 답변
Cloud:iA ISO 등록/사용 워크플로우에서 `CEPH` 타입은 지원 대상이 아닙니다.

#### 점검 포인트
1. `관리자 > 스토리지 > ISO`에서 `CEPH` 선택 불가 여부 확인
2. 프로젝트 ISO 설치 유형에서 사용 가능한 ISO 목록 확인

#### 조치 가이드
1. ISO는 `LOCAL/GFS2/NFS` 등 지원 도메인에 등록
2. CEPH 운영 환경이어도 ISO 저장은 별도 지원 도메인 분리 운영
...
### Q. 프로젝트 삭제가 되지 않아요. 시작 템플릿이나 프로젝트 이미지가 남아 있으면 왜 삭제되지 않나요?
#### 핵심 답변
프로젝트 삭제는 "프로젝트 소속 리소스 0개"일 때만 가능합니다. 리소스가 하나라도 남아 있으면 삭제가 차단됩니다.

#### 상세 점검 포인트
1. `관리자 > 프로젝트 관리 > 삭제 모달`에서 `삭제 불가능한 프로젝트` 사유 확인
2. 아래 메뉴를 순서대로 확인
   - `프로젝트 > 가상머신 > 인스턴스`
   - `프로젝트 > 가상머신 > 시작 템플릿`
   - `프로젝트 > 스토리지 > 이미지/블록/파일 시스템/스냅샷`
   - `프로젝트 > 네트워크 > 외부 IP/플로팅 IP/로드밸런서/타겟 그룹`
3. 상위 서비스 잔존 리소스 확인
   - `오토 스케일링 그룹`
   - `쿠버네티스 관련 워커/연계 리소스`
...
2. 인스턴스 종료 후 삭제
3. 스냅샷/볼륨/파일 시스템 정리
4. 시작 템플릿/프로젝트 이미지 삭제
5. 외부 IP/플로팅 IP/LB/TG 정리
6. 프로젝트 삭제 재시도
