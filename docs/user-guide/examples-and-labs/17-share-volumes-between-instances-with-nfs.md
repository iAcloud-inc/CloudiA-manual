...
4. (선택) `사용자 계정` 섹션에서 NFS 서버 로그인 계정을 지정합니다. 계정명·비밀번호·SSH 키는 모두 선택 입력이며, 비워 두면 계정 없이 생성됩니다. 이 실습은 마운트만 확인하므로 비워 두어도 됩니다.
5. `생성`을 클릭합니다.

> ℹ️ NFS 서버 VM에 직접 로그인(SSH)해야 하는 경우에만 위 사용자 계정을 지정하십시오. 자세한 규칙은 [스토리지 > 파일 시스템 - `NFS` 타입 사용자 계정](../../project/storage/file-system/file-system.md#nfs-타입-사용자-계정-선택)을 참고합니다.
