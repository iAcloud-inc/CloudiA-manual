...
이 문서는 커스텀 NACL을 생성하고 규칙을 추가한 뒤 서브넷을 연결해 트래픽 제어를 검증하는 실습입니다.

## 사전 준비
- CloudiA 웹 콘솔 접근 가능
- 프로젝트 계정(`project-owner`)으로 로그인 가능
- 퍼블릭 서브넷 + 플로팅 IP 연결 인스턴스 준비
- 보안그룹은 인바운드/아웃바운드 허용 상태로 준비

## 개요
1. [커스텀 NACL 생성](#step-1)
...
|---|---|
| 인바운드 프로토콜 | `TCP` |
| 인바운드 소스 CIDR | `203.0.113.0/24` |
| 인바운드 포트 | `22` |
| 인바운드 허용/거부 | `허용` |
| 아웃바운드 프로토콜 | `TCP (임시 포트)` |
| 아웃바운드 대상 CIDR | `203.0.113.0/24` |
| 아웃바운드 허용/거부 | `허용` |
...
### 절차
1. NACL 상세 `연결된 서브넷` 탭에서 `연결`을 클릭합니다.
...
| SSH 명령 | `ssh example-user@203.0.113.60` |
