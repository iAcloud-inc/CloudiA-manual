...
하드웨어 점검이나 펌웨어 교체를 위해 노드를 클러스터에서 잠시 떼어 두는 상태입니다. 유지관리 모드에 들어간 노드에는 새 인스턴스가 배치되지 않고, 자동 복구와 페일오버의 대상에서도 제외되며, 그 노드가 맡고 있던 워크로드는 다른 노드로 옮겨집니다(대피). 자세한 내용은 [노드 - 유지관리 모드](../administrator/host-machine/machine/machine.md#유지관리-모드)를 참고합니다.
...
이미지나 ISO 파일에 담긴 운영체제의 종류입니다. 인스턴스를 생성할 때 OS 타입에 따라 부팅 디스크의 기본 크기, 부팅 펌웨어, 드라이버 자동 연결 같은 기본 구성이 결정됩니다.
