...
* **인바운드 규칙 탭**
    * 인바운드 규칙 탭에서는 선택한 NACL에 적용되는 들어오는 트래픽 규칙 목록을 확인할 수 있습니다.
      ...
      | **포트** | 규칙이 적용될 **대상 포트 번호 또는 포트 범위**입니다. 단일 포트 번호 또는 `X:Y` 형태의 범위를 지정하며, `ALL`은 모든 포트를 의미합니다. |
    ...
    * 규칙의 정의
        ...
        * 포트 : 평가할 트래픽의 대상 포트로, TCP, UDP의 경우 사용자가 직접 포트 범위를 지정할 수 있습니다. 포트 범위는 X:Y 형태로 입력해야 하며, 단일 포트도 입력이 가능합니다. TCP/UDP 임시 포트의 경우 클라이언트에서 사용하는 임시 포트(32768:65535)로 고정되어 있습니다. ICMP 프로토콜도 "모든 유형"으로 타입이 고정되어 있습니다.
...
* **아웃바운드 규칙 탭**
    * 아웃바운드 규칙 탭에서는 해당 NACL이 적용하는 나가는 트래픽 규칙 목록을 제공합니다.
      ...
      | **포트** | 규칙이 적용되는 **대상 포트 또는 포트 범위**입니다. `ALL`은 모든 포트를 의미합니다.                     |
