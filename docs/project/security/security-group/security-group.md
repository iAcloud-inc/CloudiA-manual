...
- **ID**: 보안그룹 고유 ID입니다.
- **VPC**: 보안그룹이 속한 VPC입니다.
- **생성 방식**: 보안그룹이 만들어진 경로입니다. `수동 생성`, `오토 스케일링 그룹`, `쿠버네티스`, `VPC`, `파일 시스템` 중 하나로 표시되며, 해당하는 값이 없으면 `-`로 표시됩니다. `파일 시스템`은 NFS 파일 시스템 생성 시 자동으로 만들어진 마운트용 보안그룹(`nfs-sg-{파일 시스템 ID}`)입니다.
...
- 플로팅 IP나 포트 포워딩 외부 IP를 통해 들어오는 접속은 경로에 따라 인바운드 규칙이 판정하는 출발지가 사설 IP가 아닐 수 있습니다. 자세한 내용은 [외부 IP로 접속할 때 보안그룹이 판정하는 출발지 주소](../../network/floating-ip/floating-ip.md#외부-ip로-접속할-때-보안그룹이-판정하는-출발지-주소)를 참고하세요.
