...
- `삭제`: 선택한 보안그룹을 삭제합니다. (단, 연결된 인스턴스가 존재하지 않는 경우에만 삭제 가능합니다.)
...
| **ID**        | 보안그룹에 부여된 고유 ID입니다.              |
| **VPC** | 보안그룹이 속한 VPC 입니다.                |
| **인바운드 / 아웃바운드 규칙 수** | 보안그룹의 인바운드 규칙 수와 아웃바운드 규칙 수입니다.  |
...
### 2. 규칙 가져오기
보안그룹 템플릿과 해당하는 프로젝트의 보안그룹의 규칙을 단일하게 또는 복수 개를 가져올 수 있으며, 생성 시 필수 사항은 아닙니다.
- **이름**: 보안그룹의 이름이 표시됩니다. 
- **VPC**: 보안그룹 템플릿의 경우 '-'와 같이 표기되며, 프로젝트 하위 보안그룹의 경우 종속된 VPC가 표시됩니다.
- **인바운드 규칙 / 아웃바운드 규칙**: 인바운드와 아웃바운드의 규칙 수가 표시되며, 오른 쪽의 [보기] 버튼을 클릭하면, 하단에서 해당하는 인바운드/아웃바운드 규칙을 조회할 수 있습니다. 
- **설명**: 보안그룹의 설명이 표시됩니다. 
- **생성 일시**: 보안그룹이 생성된 일자와 시간이 표시됩니다.
- **규칙 정보**: [보기] 버튼이 제공되며, 하단에 상세하게 규칙 정보를 제공합니다. 
...
> - 보안그룹 규칙은 최대 59개까지로 60개를 초과하여 설정할 수 없습니다.  
