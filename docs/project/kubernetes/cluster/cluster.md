...
- `CA 인증서 재설정`: 새 루트 CA 인증서/개인 키를 업로드해 API 서버 인증서를 재발급합니다. 재설정 후 kubeconfig를 다시 내려받아야 합니다.

> ⚠️ 주의: CA를 재설정하면 기존 kubeconfig·클라이언트 인증서 기반 연결이 모두 끊깁니다. 예정된 작업이 없다면 트래픽 영향이 적을 때 수행하십시오.
...
### 워커 오토 스케일링 그룹 탭
클러스터 내 워커 오토 스케일링 그룹을 확인·관리합니다.

#### 주요 작업
- `생성`: 새 워커 오토 스케일링 그룹 생성 화면으로 이동합니다.
- `삭제`: 선택한 **1개** 그룹을 삭제합니다. **삭제 중** 상태는 선택할 수 없습니다.
- `상세 이동`: 목록에서 **이름**을 클릭하면 그룹 상세 화면으로 이동합니다.

#### 테이블 컬럼
| 컬럼 | 설명 |
|------|------|
| **이름** | 워커 그룹 이름입니다. 클릭 시 상세 화면으로 이동합니다. |
| **ID** | 그룹 고유 식별자입니다. |
| **인스턴스 상태** | 실행 중/전체 노드 수를 `3/5` 형식으로 표시합니다. |
| **상태** | 실행 중, 삭제 중 등 그룹 상태를 배지로 표시합니다. |
| **생성 일시** | 그룹이 생성된 날짜와 시간입니다. |

> ⚠️ 주의: 진행 중(생성/삭제) 상태에서는 삭제·편집 등 일부 작업이 비활성화됩니다.

### 네트워크 탭
정보 제공용 탭으로 **서브넷/보안 그룹/로드 밸런서** 정보를 표시합니다.
...
### 리소스 탭
#### 주요 작업
- **상단 전환**: `시스템 파드`/`디플로이먼트`/`서비스`/`데몬셋`/`이벤트` 정보를 전환합니다.
...
- **이름**: 워커 오토 스케일링 그룹 이름입니다.
- **ID**: 그룹 고유 식별자입니다.
- **클러스터 ID**: 소속 클러스터를 표시합니다.
- **상태**: 실행 중, 삭제 중 등 그룹 상태를 배지로 표시합니다.
- **인스턴스 현황**: 실행 중/전체 노드 수를 표시합니다.
- **런치 템플릿**: 사용 중인 인스턴스 템플릿 이름과 ID를 표시합니다.
- **타겟 그룹**: 연동된 경우 대상 그룹 이름을 표시합니다.
- **오토스케일링 정보**: 최소/최대/초기 노드 수와 스케일 지연 시간을 표시합니다.
- **생성 일시**: 생성된 날짜와 시간입니다.
- **설명**: 입력한 설명을 표시합니다.
...
|------|------|
| **역할** | 워커로 표시됩니다. |
| **노드 이름** | 클릭 시 VM 상세 화면으로 이동합니다. |
| **노드 ID** | 노드 고유 식별자입니다. |
| **쿠버네티스 버전** | 설치된 Kubernetes 버전입니다. |
| **IP** | 노드의 IPv4 주소입니다. |
| **인스턴스 타입** | VM 사양 정보를 표시합니다. |
| **상태** | Ready/NotReady 등 상태를 표시합니다. |
| **생성 일시** | 노드 생성 시간을 표시합니다. |

> 💡 참고: 노드 이름 클릭 시 VM 상세 화면으로 이동합니다.
...
#### 1. 기본 정보
- **이름**: 영문 소문자/숫자를 사용해 그룹 이름을 입력합니다.
- **설명**: 그룹 설명을 입력합니다(선택).

#### 2. 워커 인스턴스 템플릿
- **쿠버네티스 이미지**: OS와 Kubernetes 버전을 선택합니다.
- **인스턴스 타입**: 워커 노드에 사용할 VM 사양을 선택합니다.
- **부팅 디스크 크기**: `GiB` 단위로 입력합니다.
- **보안 그룹**: 1개 이상 선택해야 합니다.

#### 3. 오토 스케일링 설정
- **초기 노드 수**: 필수 입력, 최소 1개 이상입니다.
- **최소/최대 노드 수**: 필수 입력, 최소 1개 이상으로 설정합니다.
- **스케일 지연 시간**: 초 단위로 입력합니다.
...
#### 5. 스케일링 정책
- **확장 조건**: CPU/메모리 임계치를 설정합니다.
- **축소 조건**: 동일한 방식으로 설정합니다.

#### 입력 필드 유효성
| 필드 | 허용 값 | 검증 규칙 | 비고 |
|------|---------|----------|------|
| **이름** | 영문 소문자/숫자 | 허용 문자 외 입력 시 오류 메시지를 표시합니다. | 필수 |
| **보안 그룹** | 프로젝트 내 보안 그룹 | 1개 이상 선택해야 합니다. | 필수 |
| **초기/최소/최대 노드 수** | 정수, 최소 1 이상 | 최소/최대 범위가 초기 노드 수보다 작을 수 없습니다. | 필수 |
| **스케일 지연 시간** | 초 단위 정수 | 0 이상 값을 입력합니다. | 필수 |

## 워커 오토 스케일링 그룹 삭제
- 목록에서 삭제할 그룹을 선택한 뒤 `삭제`를 클릭합니다.
- 확인 창에서 대상 그룹을 다시 확인하고 `확인`을 선택하면 삭제가 진행됩니다.

> ⚠️ 주의: **삭제 중** 상태인 그룹은 다시 삭제할 수 없습니다.
> ⚠️ 주의: 그룹 삭제 시 해당 워커 노드 VM도 함께 제거되며 복구할 수 없습니다.
> 💡 참고: 삭제 완료까지 수 분이 소요될 수 있습니다.
