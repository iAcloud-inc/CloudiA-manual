# 파일시스템
**파일 시스템** 기능은 사용자가 클라우드 환경에서 파일 기반 저장소를 생성하고 관리할 수 있도록 돕는 기능입니다.  
...
### 주요 작업
- `생성`: 새 파일 시스템을 추가합니다.  
- `편집`: 선택한 파일 시스템을 수정합니다. (`NFS`: 이름/크기, `VIRTIOFS`: 이름)  
...
| 연결된 인스턴스 수 | 해당 파일시스템에 연결된 인스턴스의 수를 나타냅니다. |
...
- **이름**: 파일 시스템의 이름을 나타냅니다.
- **ID**: 파일 시스템의 ID를 나타냅니다.
- **타입**: 파일 시스템의 타입을 나타냅니다. (`NFS` / `VIRTIOFS`)
- **크기**: 파일 시스템의 크기를 나타냅니다.
- **연결된 인스턴스 수**: 해당 파일시스템에 연결된 인스턴스의 수를 나타냅니다.
- **생성 일시**: 파일 시스템이 생성된 일시를 나타냅니다.
...
#### 파일 시스템의 타입이 `NFS`인 경우

스토리지 도메인, 네트워크 인터페이스, 인스턴스 정보를 확인할 수 있습니다.
...
#### 파일 시스템의 타입이 `VIRTIOFS`인 경우

스토리지 도메인, 인스턴스 정보를 확인할 수 있습니다.
...
> ⚠️ **주의** : VIRTIOFS는 CEPH 스토리지 도메인을 지원하지 않습니다.

### (타입이 NFS인 경우) 네트워크 인터페이스 구성 정보
...
| **타입** | NFS / VIRTIOFS | - | 필수 |
| **크기** | 숫자 | 최소 단위 0 | 타입이 NFS인 경우 필수 |
| **크기 단위** | MB, GB | - | 타입이 NFS인 경우 필수 |
| **스토리지 도메인** | - | - | 필수, VIRTIOFS 타입의 경우 CEPH 선택 불가 |
| **VPC** | - | - | 타입이 NFS인 경우 필수 |
| **서브넷** | - | - | 타입이 NFS인 경우 필수 |
...
### 편집 시 주의 사항
- 파일 시스템 편집 페이지에서는 `NFS`는 이름과 크기를, `VIRTIOFS`는 이름만 수정할 수 있습니다.
- `NFS` 크기는 기존 값 이상만 입력 가능하며, 축소(shrink)는 지원하지 않습니다.
