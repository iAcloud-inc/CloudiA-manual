...
### 목록 화면에서 확인할 사항
- `타겟 유형`과 `연결 리소스`를 보면 현재 실제 서비스에 연결 중인지 빠르게 판단할 수 있습니다.
...
| **적용 여부**    | `적용`, `미적용`              | 필수입니다. `적용`을 선택해야 연결 대상 입력 필드가 활성화됩니다.                                                                    | 생성과 동시에 리소스 연결 여부를 결정합니다. |
...
### 삭제 전 점검 순서
1. 상세 화면에서 `타겟 유형`과 `연결 리소스`를 확인합니다.
