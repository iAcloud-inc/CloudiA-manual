...
| **L4 로드 밸런서를 경유해서**<br>(어디서 접속하든 동일) | **로드 밸런서의 내부 IP**<br>서비스 트래픽과 상태 검사 모두 |
...
로드 밸런서를 경유한 트래픽은 출발지가 **로드 밸런서의 내부 IP**로 바뀝니다. 접속한 쪽이 같은 서브넷이든, 같은 VPC의 다른 서브넷이든, VPC 바깥이든 결과는 같습니다.

내부 IP는 로드 밸런서로 **들어가는** 주소와 타겟으로 트래픽을 **넘길 때** 출발지로 쓰이는 주소가 같은 것입니다. 그래서 서비스 트래픽이든 상태 검사이든, 허용해야 할 출발지는 모두 로드 밸런서 내부 IP입니다.

로드 밸런서가 여러 개라면 각각의 내부 IP가 출발지가 됩니다. 로드 밸런서는 `네트워크 > 로드 밸런서` 목록에서, 내부 IP는 로드 밸런서 상세에서 확인합니다.

출발지가 로드 밸런서 내부 IP로 고정되므로, 출발지 주소로 접속한 쪽을 구분할 수는 없습니다. 설정 절차는 [로드 밸런서 문서](../load-balancer/load-balancer.md#로드-밸런서를-경유한-접속의-보안그룹-설정)를 참고하세요.
