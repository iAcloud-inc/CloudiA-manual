...
| **사용 IP 개수 / 전체 IP 개수**   | 현재 **사용 중인 IP 수**와 시스템 예약 IP를 제외한 **사용 가능 전체 IP 수**를 표시합니다.                   |
...
- **DNS 서버:** 서브넷에 적용된 DNS 서버 주소. 미설정 시 빈 값으로 표시됩니다.
- **사용 IP 개수 / 전체 IP 개수:** 사용 중인 IP 수와, **시스템 예약 IP를 제외한 사용 가능 전체 IP 수**   (예: `5 / 251`)
- **네트워크 접근 제어:** 적용된 NACL(네트워크 접근 제어) 이름
...
- **DNS 서버:** (선택) VPC 선택 시 해당 VPC CIDR 블록(주소 대역)의 네트워크 주소 다음 두 번째 IP가 기본값으로 자동 입력됩니다. 이 IP는 VPC 내부에서 동작하는 DNS 서버 주소입니다. 필요 시 1.1.1.1 등 다른 DNS 서버 주소로 수정하거나 공란으로 둘 수 있습니다. 서브넷 내 리소스가 도메인 이름을 해석할 때 사용할 DNS 서버 주소를 지정합니다.
- **설명:** (옵션) 설명 또는 메모 입력
...
| 필드          | 허용 값 / 범위                       | 검증 규칙                                                                                      | 비고                               |
| ------------- | ------------------------------------- | ----------------------------------------------------------------------------------------------- | ---------------------------------- |
| **이름**      | 영문/숫자/`-`/`_`, 최대 **80자**      | 공백 및 기타 특수문자 불가                                                                      | **필수**    |
...
| **서브넷 유형** | 퍼블릭 / 프라이빗                 | 유형 선택 필수                                                                                 | 생성 후 **편집 가능**              |
| **DNS 서버**  | IPv4 주소 또는 미입력                 | VPC 선택 시 기본값 자동 입력                                                                   | 선택 사항 (퍼블릭/프라이빗 무관, 공란 허용) |
...
> - 네트워크 주소 다음 **+1 ~ +3 IP** (게이트웨이 등 시스템 예약 IP)
