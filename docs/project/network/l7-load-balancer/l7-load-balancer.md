...
서비스 트래픽과 상태 검사(활성화 시) 모두 출발지가 로드 밸런서의 내부 IP 주소입니다. 상태 검사는 별도 포트 없이 **타겟 포트**(타겟에 실제로 등록된 포트)로 보내집니다.

| 방향 | 프로토콜 | 포트 | 소스 | 설명 |
| --- | --- | --- | --- | --- |
| 인바운드 | TCP | 8080 (타겟 포트) | `<로드 밸런서 내부 IP>/32` | 로드 밸런서를 경유한 서비스 트래픽 허용 |

- 표의 포트는 리스너 포트가 아니라 타겟 그룹의 **타겟 포트**입니다. 리스너가 `80`이고 타겟이 `8080`이라면 `8080`을 허용합니다. 타겟마다 다른 포트로 등록했다면 각각의 포트를 허용합니다.
- 로드 밸런서의 내부 IP 주소는 로드 밸런서 상세 화면의 `내부 IP 주소`에서 확인합니다.
- NACL은 로드 밸런서와 타겟이 서로 다른 서브넷에 있어도 적용되지 않으므로 검토 대상이 아닙니다.
...
> 💡 **증상**: 보안그룹이 막은 트래픽은 응답 없이 버려집니다. 그래서 **연결이 거부되었습니다**가 아니라 **연결 시간이 초과되었습니다**로 나타납니다. 대상 서비스가 정상 실행 중인데 접속만 안 되는 형태이므로, 서비스나 가상머신 상태를 먼저 의심하면 원인을 찾기 어렵습니다.
...
- `세션 고정성`을 활성화하면 `쿠키 유효 시간`을 입력합니다. `1-604800초(7일)` 범위로 입력합니다.
...
- `Content-Type`은 드롭다운에서 `text/plain`, `text/css`, `text/html`, `application/javascript`, `application/json` 중 하나를 선택합니다. 기본값은 `text/plain`입니다.
- `메시지 본문`을 입력합니다. 최대 1024자까지 입력할 수 있습니다.
...
### 보안 정책 검증
1. 타겟 서버 보안그룹에서 로드 밸런서의 `내부 IP 주소`가 소스로 타겟 포트에 대해 허용되어 있는지 확인합니다.
2. 상태 검사를 활성화했다면 같은 출발지(내부 IP 주소)의 타겟 포트가 허용되어 있는지 확인합니다. 상태 검사와 서비스 트래픽은 출발지가 같으므로 하나의 규칙이면 됩니다.
