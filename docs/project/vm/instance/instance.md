...
- **보안 유형**: 보안 유형을 선택합니다. 현재 설치된 환경에서 지원하는 옵션만 라디오 버튼으로 활성화됩니다(예: 선택 없음, SEV, SEV-ES).
- **인스턴스 구성**
...
> ⚠️ **주의:** **편집**
> - OS 이미지, 부트 디스크 크기, GPU 패스스루 여부는 변경이 불가합니다.

> ⚠️ **주의:** **실행 중 편집(라이브 업데이트)**
  > - vCPU/메모리 **증설만 가능**합니다. **기본 NIC 삭제** 는 불가합니다.
...
> ⚠️ **주의:** **실행 중 편집(라이브 업데이트)** - 기본 네트워크 인터페이스를 제외한 추가 가상 네트워크 인터페이스의 **증설/삭제 가능**합니다.
...
- **비밀번호**: 인스턴스의 가상 OS 로그인 비밀번호를 설정합니다. *(선택 입력, SSH 키 미선택 시 필수)*
- **SSH 키**: 사전에 등록된 SSH 키를 다중 선택합니다. 최대 5개까지 선택 가능하며, 비밀번호를 입력하지 않을 경우 최소 1개 이상 지정해야 합니다. 
...
### 6. 가상 CPU 피닝 설정 
- **가상 CPU 피닝**: 가상 머신의 vCPU를 호스트의 논리 CPU에 피닝(고정)합니다. **`추가`** 버튼을 눌러 피닝 규칙(vCPU와 논리 CPU 범위)을 입력할 수 있습니다. 각 규칙에는 하나의 vCPU ID와 해당 vCPU를 고정할 **논리 CPU 범위(시작~끝)**를 지정합니다. 여러 vCPU에 대해 각각 규칙을 추가해 설정할 수 있습니다. 
