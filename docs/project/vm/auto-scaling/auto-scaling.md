...
- `삭제`: 선택한 오토 스케일링 그룹을 삭제합니다. `실행 중`, `일시 중지됨` 상태에서 활성화됩니다.
- `재개`: 선택한 오토 스케일링 그룹의 자동 용량 조정을 재개합니다. `일시 중지됨` 상태이고 자동 용량 조정이 중지된 오토 스케일링 그룹에 대해서 해당 기능이 활성화됩니다.
- `일시 중지`: 선택한 오토 스케일링 그룹의 자동 용량 조정을 일시 중지합니다. `정상 실행 중`, `스케일 아웃 진행 중`, `스케일 인 진행 중` 상태이고 자동 용량 조정이 동작 중인 오토 스케일링 그룹, 그리고 `업데이트 중` 상태의 오토 스케일링 그룹에 대해서 해당 기능이 활성화됩니다.
...
| **상태** | 오토 스케일링 그룹의 상태 아이콘과 텍스트를 표시합니다. 예를 들어 `생성 중`, `정상 실행 중`, `업데이트 중`, `삭제 중`, `일시 중지됨`, `스케일 인 진행 중`, `스케일 아웃 진행 중` 등의 상태를 아이콘과 텍스트로 보여줍니다. |
...
> 💡 **참고:** 진행 상태 회수
>
> 서비스 재배포나 프로세스 종료로 작업이 유실되면 그룹이 `스케일 아웃 진행 중`·`스케일 인 진행 중`·`업데이트 중` 상태에 멈춘 채 편집·삭제가 막힐 수 있습니다. 이 상태에서도 `일시 중지`는 활성화되므로, `일시 중지`로 상태를 `일시 중지됨`으로 되돌린 뒤 필요하면 `재개`합니다. 스케일 진행 중 상태의 고착은 서버가 일정 시간이 지나면 자동으로 회수하기도 합니다.
