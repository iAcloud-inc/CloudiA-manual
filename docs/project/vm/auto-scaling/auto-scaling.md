...
| **상태** | 오토 스케일링 그룹의 상태 아이콘과 텍스트를 표시합니다. 예를 들어 `생성 중`, `실행 중`, `업데이트 중`, `삭제 중`, `일시 중지`, `스케일 인`, `스케일 아웃` 등의 상태를 아이콘과 텍스트로 보여줍니다. |
...
> ⚠️ **주의:** 편집 화면에서는 `타겟 그룹 생성 여부`, `프로토콜`, `포트 번호` 항목이 표시되더라도 비활성화되어 변경할 수 없습니다.

### 기본 정보
- **이름** : 오토 스케일링 그룹의 이름을 지정합니다.
- **시작 템플릿** : 오토 스케일링 그룹에서 가상 머신 인스턴스 생성 시 사용할 시작 템플릿을 선택합니다.
- **타겟 그룹** : 로드 밸런서와 연동할 자세 타겟 그룹을 생성할 것인지 결정합니다. (해당 타겟 그룹에 오토 스케일링 그룹에서 관리하는 가상 머신 인스턴스들이 자동으로 등록됩니다.)
...
> ⚠️ **주의:** 편집 화면에서는 `타겟 그룹 생성 여부`, `프로토콜`, `포트 번호` 항목을 변경할 수 없습니다.
...
- **원하는 용량** : 처음에 실행할 인스턴스 개수를 설정합니다. 오토 스케일링 그룹 생성 시 해당 개수의 인스턴스를 시작합니다. 이후 오토 스케일링에 의해 인스턴스 수가 동적으로 변경될 수 있습니다.
...
### 상태 검사
- **스케일 간격** : 오토 스케일링 그룹이 인스턴스를 추가하거나 제거한 후 다음 용량 조정을 허용할 시간 간격을 입력합니다. 해당 기간 동안에는 추가적인 용량 조정을 수행하지 않으며, 이 기간이 지나지 않은 상태에서 `원하는 용량`을 변경하면 오류와 함께 거부됩니다.
...
| 스케일 간격 | 정수 | 최소 180초 이상(편집 시 기존 180초 미만 값을 그대로 두면 유지됨) | 필수 |
| 지표 유형 | CPU 평균 사용량 또는 메모리 평균 사용량 선택 | - | 필수 |
| 스케일 인 대상값 | 정수 | 0 ~ 100 (%) | 필수 |
| 스케일 아웃 대상값 | 정수 | 0 ~ 100 (%)| 스케일 인 대상값 초과, 필수 |
