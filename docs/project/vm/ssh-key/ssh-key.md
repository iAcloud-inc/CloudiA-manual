...
> ⚠️ **주의:** 연결된 인스턴스 수가 1 이상이면 `삭제`가 불가능합니다. 

---

## SSH 키 생성·편집
> 참고: 생성과 편집은 동일한 양식을 사용하며, **편집 모드**에서는 공개키 수정이 불가능합니다.
...
> 💡**참고:** `편집` 화면에서는 서버에서 내려준 공개키 원문을 확인만 가능합니다.
...
> ⚠️ **주의:** SSH 키가 인스턴스에 연결되어 있을 경우, "삭제 불가능한 SSH 키" 메시지를 표시하고 **`삭제`** 버튼이 비활성화됩니다.
